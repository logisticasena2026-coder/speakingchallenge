...
export function FormularioFraseAdmin({ onSubmitAction, modo, defaultValues }: Props) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FraseFormData>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-2xl">
      <div>
        ...
        <textarea {...register('fraseIngles')} rows={3} className="w-full bg-surface-2 border border-border-subtle rounded-lg py-3 px-4 text-text-primary focus:ring-1 focus:ring-brand-purple transition-all resize-none" />
        {errors.fraseIngles && <p className="text-xs text-red-400 mt-1">{errors.fraseIngles.message}</p>}
      </div>
      <div>
        <label className="block font-ui text-[11px] font-semibold uppercase tracking-wider text-text-muted mb-1.5">Español</label>
        <textarea {...register('fraseEspanol')} rows={3} className="w-full bg-surface-2 border border-border-subtle rounded-lg py-3 px-4 text-text-primary focus:ring-1 focus:ring-brand-purple transition-all resize-none" />
        {errors.fraseEspanol && <p className="text-xs text-red-400 mt-1">{errors.fraseEspanol.message}</p>}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <input {...register('dificultad')} type="number" min={1} max={10} className="w-full bg-surface-2 border border-border-subtle rounded-lg py-3 px-4 text-text-primary focus:ring-1 focus:ring-brand-purple transition-all" />
        </div>
        <div>
          <label className="block font-ui text-[11px] font-semibold uppercase tracking-wider text-text-muted mb-1.5">Temática</label>
          <input {...register('tematica')} className="w-full bg-surface-2 border border-border-subtle rounded-lg py-3 px-4 text-text-primary focus:ring-1 focus:ring-brand-purple transition-all" placeholder="ej: Viajes" />
        </div>
        <div>
          <label className="block font-ui text-[11px] font-semibold uppercase tracking-wider text-text-muted mb-1.5">Edad</label>
          <input {...register('edad')} type="number" min={5} max={99} className="w-full bg-surface-2 border border-border-subtle rounded-lg py-3 px-4 text-text-primary focus:ring-1 focus:ring-brand-purple transition-all" placeholder="Opcional" />
        </div>
      </div>
